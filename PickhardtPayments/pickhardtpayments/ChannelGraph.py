...
class ChannelGraph:
    ...
    def close_channels_up_to_amount(self, node: str, threshold_to_reach: float, closing_strategy: str):
        ...
        for dest in result:
            ...
            self.close_channel(src=node, dest=dest, short_channel_id=c.short_channel_id)
    ...
    def getDiGraph(self, amount: int, mu: int, amt = 10_000):
        """
        params:
        ...
        for u, v, keys, channel in self.network.edges(data="channel", keys=True):
            ...
            if not G_DiGraph.has_edge(u, v):
                G_DiGraph.add_edge(u, v,
                                ...
                                fee = int(( channel.ppm * amt/1000 + channel.base_fee )) + 1,
