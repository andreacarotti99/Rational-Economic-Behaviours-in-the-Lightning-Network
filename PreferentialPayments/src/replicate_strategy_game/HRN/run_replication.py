...
alfas = [0.0, 0.1]
high_cap_nodes = [11, 13, 21, 27]  # these are the highest ratio nodes that in a simulation of 10k trans w. alfa=0.0 routed
...
def run_script(high_cap_node, alfa):
    ...
    # replicate_node(channel_graph=channel_graph, node_to_copy='A', copier='C')
    ...
    for i in range(hrn_to_replicate):
        ...
        target = get_nth_hrn_from_results(export_filename, target_idx+1)
        while cg.get_expected_capacity(agent) <= cg.get_expected_capacity(target) and simulation_0.nodes_routing[target] < 10:
            target_idx += 1
            target = get_nth_hrn_from_results(export_filename, target_idx+1)
            if target_idx == 1000:
...
# print(simulation.nodes_earning)
