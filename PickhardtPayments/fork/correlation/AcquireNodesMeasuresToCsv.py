...
from pickhardtpayments.fork.correlation import centrality_func
...
G_DiGraph_mu0 = channel_graph.getDiGraph(amount=10_000, mu=0)  # NB: the degree() function is broken on this graph...
G_DiGraph_mu1 = channel_graph.getDiGraph(amount=10_000, mu=1)  # NB: the degree() function is broken on this graph...
G_DiGraph_mu10 = channel_graph.getDiGraph(amount=10_000, mu=10)  # NB: the degree() function is broken on this graph...
G_DiGraph_mu100 = channel_graph.getDiGraph(amount=10_000, mu=100)  # NB: the degree() function is broken on this graph...
G_DiGraph_mu1000 = channel_graph.getDiGraph(amount=10_000, mu=1000)  # NB: the degree() function is broken on this graph...
...
ec_pickhardt = nx.eigenvector_centrality(G_DiGraph_mu1000, weight='pickhardt_weight', max_iter=400, tol=1024 * 1.0e-6)
...
bc_standard = nx.betweenness_centrality(G_DiGraph_mu1000, normalized=True)
...
ec_standard = nx.eigenvector_centrality(G_DiGraph_mu1000)
...
closeness_standard = nx.closeness_centrality(G_DiGraph_mu1000)
...
bc_ppm = nx.betweenness_centrality(G_DiGraph_mu1000, weight='ppm', normalized=True)
...
bc_base = nx.betweenness_centrality(G_DiGraph_mu1000, weight='base', normalized=True)
...
bc_capacity = nx.betweenness_centrality(G_DiGraph_mu1000, weight='capacity', normalized=True)
...
bc_pickhardt = nx.betweenness_centrality(G_DiGraph_mu1000, weight='pickhardt_weight', normalized=True)
df_bc_pickhardt = pd.DataFrame(list(bc_pickhardt.items()), columns=['node', 'bc_pickhardt'])

# df with the betwenness centrality with respect to the pickhardt weight
print("Computing betwenness centrality (edges weight: final fee weight)...")
bc_pickhardt = nx.betweenness_centrality(G_DiGraph_mu1000, weight='fee', normalized=True)
df_bc_pickhardt = pd.DataFrame(list(bc_pickhardt.items()), columns=['node', 'bc_final_fee'])
...
df_degree = pd.DataFrame(list(centrality_func.compute_degrees_at_distance(G_DiGraph_mu1000, radius=1).items()),
                         columns=['node', 'degree'])

# df with the "degree two hops" / ego distance 2 of all the nodes
print("Computing degree (2 hop distance)...")
df_degree_distance_2 = pd.DataFrame(list(centrality_func.compute_degrees_at_distance(G_DiGraph_mu1000, radius=2).items()),
                                    columns=['node', 'degree_2_hops'])

# df with the expected capacity of each node
print("Computing expected capacity...")
df_exp_cap = pd.DataFrame(list(channel_graph.get_nodes_capacities().items()), columns=['node', 'capacity'])
