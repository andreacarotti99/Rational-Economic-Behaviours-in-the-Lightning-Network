import pandas as pd
from identify_HRN import get_nth_hcn_from_results, get_ratio_of_node, get_fees_of_node  # Import necessary functions
...
base_path = "../../../src/replicate_strategy_game/HRN/results/closing channels randomly 1000 trans hcn 1 - 11 - 13 - 21 - 27 - 28/"
alfa = 0.1
...
def generate_file_path(sim_index, hcn_rank, alfa_value):
    if sim_index == 0:
        ...
        f = f"{base_path}hcn{hcn_rank}/alfa{alfa_value}/results_1000trans_10000SAT_{alfa_value}alfa_unifdist_0hcn{hcn_rank}_alfa{alfa_value}.csv"
        #print(f)
        return f
    else:
        # General case for other simulations
        f = f"{base_path}hcn{hcn_rank}/alfa{alfa_value}/results_1000trans_10000SAT_{alfa_value}alfa_unifdist_{sim_index}_duplAlreadyPresentTrue_hcn{hcn_rank}.csv"
        #print(f)
        return f
