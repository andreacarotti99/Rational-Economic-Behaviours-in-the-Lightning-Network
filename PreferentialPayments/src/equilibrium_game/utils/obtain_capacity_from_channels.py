...
def obtain_capacity_from_channels(G: ChannelGraph, node: str, amt: int):
    ...
    print(f"\tObtaining {amt} / {len(list(G.network.out_edges(node)))} = {cap_for_each_edge_to_get} from each channel")

    if amt > G.get_expected_capacity(node):
        print(f"Node {node} has a capacity of {G.get_expected_capacity(node)} < {amt} I cannot open a channel with this node")
        return False
    for dest_node in G.get_connected_nodes(node):
        for ch in G.get_channels(node, dest_node):
            ...
            print(f"\t|__ updated channel of cap: {ch.capacity} - {cap_for_each_edge_to_get} = {ch.capacity - cap_for_each_edge_to_get}")
    return True
